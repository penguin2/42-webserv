...
from presentation.content_creator import create_book_detail_html
import cgi
import os
...
def generate_book_detail_page(user: tuple, session_id: str, max_age: int = 30):
    ...
    try:
        ...
        book = books[0]
        book_detail = create_book_detail_html(book)
    except Exception:
        prev_page = os.environ.get("HTTP_REFERRER")
        if prev_page is None:
            prev_page = "/42library/index.py"
        book_detail = f"""
            <p style="text-align:center; font-size:50px;">
                Book is not exist.
            </p>
            <a href={prev_page} style="text-align:center; font-size:50px;">
                <p>RETURN</p>
            </a>
        """
