...
def test_file_not_exist():
    request_by_delete("/no_exist_file", 404)


def test_delete_dir():
    mkdir_and_delete("/directory", 404)


def test_no_permission_file():
    delete_file_without_permission("/no_permission", 404)


def test_no_permission_dir():
    delete_file_in_dir_without_permission("/dir", "/file", 404)
