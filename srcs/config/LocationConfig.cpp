...
void LocationConfig::setTryFilesPath(const std::string& newPath) { try_files_path = newPath; }
...
void LocationConfig::setAutoindex(bool newAutoindex) { autoindex = newAutoindex; }


const std::vector<HttpMethod>& LocationConfig::getAllowMethods() const {
  return allow_methods;
}
void LocationConfig::setAllowMethods(const std::vector<HttpMethod> new_methods) {
...
void LocationConfig::setCgiExt(const std::string& newExt) {
    cgi_ext = newExt;
}
